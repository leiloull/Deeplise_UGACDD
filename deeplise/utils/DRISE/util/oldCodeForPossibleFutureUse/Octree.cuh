...
public:
  ...
  Unity* cloud;//Sphere or float3
  Geometry cloud_type;

  Unity* normals;//float3
  Unity* colors;//uchar3 or uchar

  Unity* nodes;//Node
  Unity* vertices;//Vertex
  Unity* edges;//Edge`
  Unity* faces;//Face

  //length = # points, value = node containing point
  //ie value = index of node point is in
  Unity* pointNodeIndex;//unsigned int

  //depth index carriers
  Unity* nodeDepthIndex;//unsigned int
  Unity* vertexDepthIndex;//unsigned int
  Unity* edgeDepthIndex;//unsigned int
  Unity* faceDepthIndex;//unsigned int

  /*
  SPLIT DATASET (SPHERES ONLY RIGHT NOW)
  */
  ...
  Octree(int numPoints, float3* points, int depth, bool createVEF);
  Octree(int numPoints, float3* points, float deepestWidth, bool createVEF);
  Octree(int numSpheres, Sphere* spheres, int depth, bool createVEF);
  Octree(int numSpheres, Sphere* spheres, float deepestWidth, bool createVEF);

  Octree(Unity* cloud, Geometry cloud_type, int depth, bool createVEF);
  Octree(Unity* cloud, Geometry cloud_type, float deepestWidth, bool createVEF);
