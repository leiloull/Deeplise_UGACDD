...
def determineBindingSite(proteinJson):
    ...
    numClusters = int(totalAtoms/100)

    if numClusters < 5:
        print('Min Detected')
        numClusters = 5

    kMeans1 = KMeans(n_clusters=numClusters, n_init=6, max_iter=1000, random_state=0).fit(proteinArray)
    ...
    kMeans2 = KMeans(n_clusters=2, n_init=6, max_iter=1000, random_state=0).fit(groupAverages.reshape(-1, 1))
